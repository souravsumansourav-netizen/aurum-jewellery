  ...
  return (
    <div className="space-y-4">
      <div className="bg-sienna/10 border border-sienna/20 p-5">
        ...
        <p className="font-sans text-[11px] tracking-[0.15em] uppercase text-charcoal font-medium">{result.partialStore.name}</p>
          ...
          {cartProducts.map(p => {
            ...
            return (
              <div key={p.id} className="flex items-center gap-2">
                <span className={available ? 'text-gold' : 'text-sienna'}>{available ? '✓' : '✗'}</span>
      ...
      {result.fullStore && (
        <>
          <div className="flex items-center gap-3">
            ...
            <span className="font-serif italic text-xs text-gold whitespace-nowrap">Better option — 100% available</span>
            <div className="flex-1 h-px bg-gold/20" />
          </div>
          <div className="bg-gold-light border border-gold/30 p-5">
            ...
            <p className="font-sans text-[11px] tracking-[0.15em] uppercase text-charcoal font-medium">{result.fullStore.name}</p>
